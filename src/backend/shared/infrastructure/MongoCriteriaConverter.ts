...
export class MongoCriteriaConverter {
    ...
    protected generateFilter(filters: Filters): MongoFilter {
        ...
        // This is necesary because with duplicate document keys, Object.assign overrides the previous values
        return filter.reduce((mergedQuery, filter) => {
            Object.entries(filter).forEach(([key, value]) => {
                mergedQuery[key] = mergedQuery[key] ? { ...mergedQuery[key], ...value } : value;
            });
            return mergedQuery;
        }, {});
    }
